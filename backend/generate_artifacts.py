...
Uses the selected LinearRegression model to generate:
1. predicted_vs_actual.png
2. residual_distribution.png
3. feature_importance.png (Permutation Importance)
4. coefficient_importance.png & coefficient_analysis.json (Model Coefficients)
5. model_comparison.png & model_comparison.json
"""
...
def generate_all_artifacts():
    ...
    # 2. Train selected LinearRegression model
    ...
    # 3. Model Coefficient Analysis
    logger.info("Computing Linear Regression Coefficients...")
    coef_dict = []
    for feat, coef in zip(FEATURE_COLUMNS, model.coef_):
        coef_dict.append({
            "feature": feat,
            "raw_coefficient": round(float(coef), 4),
            "feature_scale_std": round(float(X[feat].std()), 4),
            "standardized_effect": round(float(coef * X[feat].std()), 4)
        })
    coef_dict = sorted(coef_dict, key=lambda x: abs(x["standardized_effect"]), reverse=True)

    with open(os.path.join(ARTIFACTS_DIR, "coefficient_analysis.json"), "w") as f:
        json.dump({
            "intercept": round(float(model.intercept_), 4),
            "coefficients": coef_dict
        }, f, indent=2)

    # Plot: Coefficient Standardized Effects
    plt.figure(figsize=(9, 5))
    coef_feats = [c["feature"] for c in coef_dict]
    coef_effects = [c["standardized_effect"] for c in coef_dict]
    colors = ["#2563EB" if val >= 0 else "#DC2626" for val in coef_effects]
    plt.barh(coef_feats[::-1], coef_effects[::-1], color=colors[::-1])
    plt.xlabel("Standardized Impact on Virality Score (Beta * StdDev)", fontsize=11, fontweight="bold")
    plt.title("Linear Regression Standardized Feature Effects", fontsize=13, fontweight="bold", pad=12)
    plt.grid(axis="x", linestyle="--", alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(ARTIFACTS_DIR, "coefficient_importance.png"), dpi=200)
    plt.close()

    # 4. Permutation Feature Importance on LinearRegression
    logger.info("Computing Permutation Feature Importance for Linear Regression...")
    ...
    # Plot 1: Feature Importance (Permutation on LinearRegression)
    plt.figure(figsize=(9, 5))
    ...
    plt.title("Permutation Feature Importance — Linear Regression Baseline", fontsize=13, fontweight="bold", pad=12)
    plt.grid(axis="x", linestyle="--", alpha=0.6)
    plt.tight_layout()
    ...
    plt.title(f"Predicted vs Actual Virality Score\n(Linear Regression: R² = {r2:.4f}, RMSE = {rmse:.4f})", fontsize=13, fontweight="bold", pad=12)
    ...
    ax1.set_xticks(x)
    ax1.set_xticklabels(m_names, rotation=15, ha="right", fontsize=8.5, fontweight="bold")
